...
static int32_t get_bucket_index(const struct hdr_histogram* h, int64_t value) {
  int32_t pow2ceiling =
      64 -
      count_leading_zeros_64(value | h->sub_bucket_mask); /* smallest power of 2 containing value */
...
int64_t hdr_size_of_equivalent_value_range(const struct hdr_histogram* h, int64_t value) {
  ...
  int32_t adjusted_bucket =
      (sub_bucket_index >= h->sub_bucket_count) ? (bucket_index + 1) : bucket_index;
...
static int32_t buckets_needed_to_cover_value(int64_t value, int32_t sub_bucket_count,
                                             int32_t unit_magnitude) {
...
int hdr_calculate_bucket_config(int64_t lowest_trackable_value, int64_t highest_trackable_value,
  ...
  sub_bucket_count_magnitude =
      (int32_t)ceil(log((double)largest_value_with_single_unit_resolution) / log(2));
...
int hdr_init(int64_t lowest_trackable_value, int64_t highest_trackable_value,
  ...
  int r = hdr_calculate_bucket_config(lowest_trackable_value, highest_trackable_value,
...
bool hdr_record_corrected_values(struct hdr_histogram* h, int64_t value, int64_t count,
                                 int64_t expected_interval) {
...
int64_t hdr_add_while_correcting_for_coordinated_omission(struct hdr_histogram* h,
...
void hdr_iter_percentile_init(struct hdr_iter* iter, const struct hdr_histogram* h,
...
void hdr_iter_linear_set_value_units_per_bucket(struct hdr_iter* iter,
                                                int64_t value_units_per_bucket) {
  iter->specifics.linear.value_units_per_bucket = value_units_per_bucket;
}
...
int hdr_percentiles_print(struct hdr_histogram* h, FILE* stream, int32_t ticks_per_half_distance,
  ...
  if (CLASSIC == format) {
    ...
    if (fprintf(stream, CLASSIC_FOOTER, mean, stddev, max, h->total_count, h->bucket_count,
