...
static inline double pt_norm(double p) {
  ...
  if (fabs(q) <= 0.425) {
    ...
    if (r <= 5.0) {
      ...
      r -= 1.6;
      ret = ((((((((7.74545014278341407640e-4 * r + 2.27238449892691845833e-2) * r +
      ...
      /* p near 0 or 1. */
      r -= 5.0;
      ret = ((((((((2.01033439929228813265e-7 * r + 2.71155556874348757815e-5) * r +
...
static inline double pt_chi2(double p, double df, double ln_gamma_df_2) {
  ...
  for (i = 0; i < 20; i++) {
    ...
    ch += t *
          (1.0 + 0.5 * t * s1 - b * c * (s1 - b * (s2 - b * (s3 - b * (s4 - b * (s5 - b * s6))))));
    if (fabs(q / ch - 1.0) <= e) {
      break;
    }
  }
