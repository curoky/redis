...
TEST_BEGIN(test_small_extent_size) {
  ...
  for (i = 0; i < nbins; i++) {
    mib[2] = i;
    sz = sizeof(size_t);
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&extent_size, &sz, NULL, 0), 0,
                "Unexpected mallctlbymib failure");
    assert_zu_eq(extent_size, extent_size_quantize_floor(extent_size),
                 "Small extent quantization should be a no-op "
                 "(extent_size=%zu)",
                 extent_size);
    assert_zu_eq(extent_size, extent_size_quantize_ceil(extent_size),
                 "Small extent quantization should be a no-op "
                 "(extent_size=%zu)",
                 extent_size);
  }
...
TEST_BEGIN(test_large_extent_size) {
  ...
  sz = sizeof(bool);
  assert_d_eq(mallctl("config.cache_oblivious", (void *)&cache_oblivious, &sz, NULL, 0), 0,
              "Unexpected mallctl failure");

  sz = sizeof(unsigned);
  assert_d_eq(mallctl("arenas.nlextents", (void *)&nlextents, &sz, NULL, 0), 0,
              "Unexpected mallctl failure");
  ...
  for (i = 0; i < nlextents; i++) {
    ...
    mib[2] = i;
    sz = sizeof(size_t);
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&lextent_size, &sz, NULL, 0), 0,
                "Unexpected mallctlbymib failure");
    ...
    if (i > 0) {
      assert_zu_eq(extent_size_prev, extent_size_quantize_floor(extent_size - PAGE),
...
#define SZ_MAX ZU(4 * 1024 * 1024)
  ...
  for (i = 1; i <= SZ_MAX >> LG_PAGE; i++) {
    ...
    assert_zu_le(floor, extent_size, "Floor should be <= (floor=%zu, extent_size=%zu, ceil=%zu)",
...
int main(void) { return test(test_small_extent_size, test_large_extent_size, test_monotonic); }
