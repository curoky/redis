...
TEST_BEGIN(test_tsd_main_thread) { thd_start((void *)(uintptr_t)0xa5f3e329); }
TEST_END
...
static void *thd_start_reincarnated(void *arg) {
  ...
  assert_u_eq(tsd->state, tsd_state_purgatory, "TSD state should be purgatory\n");

  free(p);
  assert_u_eq(tsd->state, tsd_state_reincarnated, "TSD state should be reincarnated\n");
...
int main(void) {
  ...
  return test_no_reentrancy(test_tsd_main_thread, test_tsd_sub_thread, test_tsd_reincarnation);
}
