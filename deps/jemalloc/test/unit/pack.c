...
static unsigned binind_compute(void) {
  ...
  for (i = 0; i < nbins; i++) {
    ...
    sz = sizeof(size);
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&size, &sz, NULL, 0), 0,
...
static size_t nregs_per_run_compute(void) {
  ...
  mib[2] = (size_t)binind;
  sz = sizeof(nregs);
  assert_d_eq(mallctlbymib(mib, miblen, (void *)&nregs, &sz, NULL, 0), 0,
              "Unexpected mallctlbymib failure");
  return nregs;
}
...
int main(void) { return test(test_pack); }
