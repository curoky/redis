...
static void test_bitmap_initializer_body(const bitmap_info_t *binfo, size_t nbits) {
              ...
              nbits);
  {
    ...
    for (i = 0; i < binfo->nlevels; i++) {
      assert_zu_eq(binfo->levels[i].group_offset, binfo_dyn.levels[i].group_offset,
...
static void test_bitmap_xfu_body(const bitmap_info_t *binfo, size_t nbits) {
  ...
  assert_zu_eq(bitmap_ffu(bitmap, binfo, (nbits > 1) ? nbits - 2 : nbits - 1), nbits - 1,
...
int main(void) {
  return test(test_bitmap_initializer, test_bitmap_size, test_bitmap_init, test_bitmap_set,
              test_bitmap_unset, test_bitmap_xfu);
}
