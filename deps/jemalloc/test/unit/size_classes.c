...
static size_t get_max_size_class(void) {
  ...
  sz = sizeof(size_t);
  assert_d_eq(mallctlbymib(mib, miblen, (void *)&max_size_class, &sz, NULL, 0), 0,
...
TEST_BEGIN(test_size_classes) {
  ...
  for (index = 0, size_class = sz_index2size(index);
    ...
    assert_zu_eq(size_class, sz_index2size(sz_size2index(size_class)),
    ...
    assert_zu_eq(size_class, (index > 0) ? sz_s2u(sz_index2size(index - 1) + 1) : sz_s2u(1),
  ...
  assert_zu_eq(max_size_class, sz_index2size(sz_size2index(max_size_class)),
...
TEST_BEGIN(test_psize_classes) {
  ...
  for (pind = 0, size_class = sz_pind2sz(pind); pind < max_pind || size_class < max_psz;
    ...
    assert_zu_eq(size_class, (pind > 0) ? sz_psz2u(sz_pind2sz(pind - 1) + 1) : sz_psz2u(1),
...
TEST_BEGIN(test_overflow) {
  ...
  assert_u_eq(sz_psz2ind(ZU(PTRDIFF_MAX) + 1), NPSIZES,
              "sz_psz2ind() should return NPSIZES on overflow");
  assert_u_eq(sz_psz2ind(SIZE_T_MAX), NPSIZES, "sz_psz2ind() should return NPSIZES on overflow");
...
int main(void) { return test(test_size_classes, test_psize_classes, test_overflow); }
