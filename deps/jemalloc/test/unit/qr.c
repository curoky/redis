...
static void test_independent_entries(ring_t *entries) {
  ...
  for (i = 0; i < NENTRIES; i++) {
    j = 0;
    qr_reverse_foreach(t, &entries[i], link) { j++; }
    assert_u_eq(j, 1,
                "Iteration over single-element ring should visit precisely "
                "one element");
  }
  ...
  for (i = 0; i < NENTRIES; i++) {
    ...
    qr_foreach(t, &entries[i], link) {
      if (i < SPLIT_INDEX) {
        ...
        assert_c_eq(t->id,
                    entries[(i + j - SPLIT_INDEX) % (NENTRIES - SPLIT_INDEX) + SPLIT_INDEX].id,
                    "Element id mismatch");
      }
      j++;
    }
...
int main(void) {
  return test(test_qr_one, test_qr_after_insert, test_qr_remove, test_qr_before_insert,
              test_qr_meld_split);
}
