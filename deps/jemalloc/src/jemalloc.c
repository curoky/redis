...
JEMALLOC_ATTR(used) static const void(WINAPI *init_init_lock)(void) = _init_init_lock;
#endif
#endif
#else
...
bool malloc_initialized(void) { return (malloc_init_state == malloc_init_initialized); }
...
static void *a0ialloc(size_t size, bool zero, bool is_internal) {
  ...
  return iallocztm(TSDN_NULL, size, sz_size2index(size), zero, NULL, is_internal,
...
void *a0malloc(size_t size) { return a0ialloc(size, false, true); }

void a0dalloc(void *ptr) { a0idalloc(ptr, true); }
...
static void narenas_total_inc(void) { atomic_fetch_add_u(&narenas_total, 1, ATOMIC_RELEASE); }

unsigned narenas_total_get(void) { return atomic_load_u(&narenas_total, ATOMIC_ACQUIRE); }
...
arena_t *arena_choose_hard(tsd_t *tsd, bool internal) {
  ...
  if (narenas_auto > 1) {
    ...
    for (j = 0; j < 2; j++) {
      if (arena_nthreads_get(arena_get(tsd_tsdn(tsd), choose[j], false), !!j) == 0 ||
          first_null == narenas_auto) {
        /*
        ...
        choose[j] = first_null;
        arena =
            arena_init_locked(tsd_tsdn(tsd), choose[j], (extent_hooks_t *)&extent_hooks_default);
        if (arena == NULL) {
  ...
  if (config_stats) {
    ...
    for (i = 0, narenas = narenas_total_get(); i < narenas; i++) {
      ...
      if (arena != NULL) {
        ...
        ql_foreach(tcache, &arena->tcache_ql, link) { tcache_stats_merge(tsdn, tcache, arena); }
...
static bool malloc_conf_next(char const **opts_p, char const **k_p, size_t *klen_p,
...
static void malloc_conf_error(const char *msg, const char *k, size_t klen, const char *v,
                              size_t vlen) {
...
static void malloc_slow_flag_init(void) {
  ...
  malloc_slow_flags |= (opt_junk_alloc ? flag_opt_junk_alloc : 0) |
                       (opt_junk_free ? flag_opt_junk_free : 0) | (opt_zero ? flag_opt_zero : 0) |
                       (opt_utrace ? flag_opt_utrace : 0) | (opt_xmalloc ? flag_opt_xmalloc : 0);
...
#define CONF_HANDLE_BOOL(o, n)                                   \
  if (CONF_MATCH(n)) {                                           \
    if (CONF_MATCH_VALUE("true")) {                              \
      ...
      malloc_conf_error("Invalid conf value", k, klen, v, vlen); \
    }                                                            \
    continue;                                                    \
  }
...
#define CONF_HANDLE_T_U(t, o, n, min, max, check_min, check_max, clip)                \
  if (CONF_MATCH(n)) {                                                                \
    ...
    if (get_errno() != 0 || (uintptr_t)end - (uintptr_t)v != vlen) {                  \
      ...
      if (CONF_MIN_##check_min(um, (t)(min))) {                                       \
        o = (t)(min);                                                                 \
      } else if (CONF_MAX_##check_max(um, (t)(max))) {                                \
        o = (t)(max);                                                                 \
      } else {                                                                        \
        o = (t)um;                                                                    \
      }                                                                               \
    } else {                                                                          \
      if (CONF_MIN_##check_min(um, (t)(min)) || CONF_MAX_##check_max(um, (t)(max))) { \
        malloc_conf_error(                                                            \
            "Out-of-range "                                                           \
            "conf value",                                                             \
            k, klen, v, vlen);                                                        \
...
#define CONF_HANDLE_SSIZE_T(o, n, min, max)                           \
  if (CONF_MATCH(n)) {                                                \
    ...
    if (get_errno() != 0 || (uintptr_t)end - (uintptr_t)v != vlen) {  \
      ...
      malloc_conf_error("Out-of-range conf value", k, klen, v, vlen); \
    } else {                                                          \
      o = l;                                                          \
    }                                                                 \
    continue;                                                         \
...
#define CONF_HANDLE_CHAR_P(o, n, d)                                 \
  if (CONF_MATCH(n)) {                                              \
    ...
    continue;                                                       \
        ...
        for (i = 0; i < dss_prec_limit; i++) {
          if (strncmp(dss_prec_names[i], v, vlen) == 0) {
            if (extent_dss_prec_set(i)) {
              malloc_conf_error("Error setting dss", k, klen, v, vlen);
            } else {
      ...
      CONF_HANDLE_UNSIGNED(opt_narenas, "narenas", 1, UINT_MAX, yes, no, false)
      CONF_HANDLE_SSIZE_T(
          opt_dirty_decay_ms, "dirty_decay_ms", -1,
          NSTIME_SEC_MAX * KQU(1000) < QU(SSIZE_MAX) ? NSTIME_SEC_MAX * KQU(1000) : SSIZE_MAX);
      CONF_HANDLE_SSIZE_T(
          opt_muzzy_decay_ms, "muzzy_decay_ms", -1,
          NSTIME_SEC_MAX * KQU(1000) < QU(SSIZE_MAX) ? NSTIME_SEC_MAX * KQU(1000) : SSIZE_MAX);
      ...
      CONF_HANDLE_SIZE_T(opt_lg_extent_max_active_fit, "lg_extent_max_active_fit", 0,
        ...
        for (int i = percpu_arena_mode_names_base; i < percpu_arena_mode_names_limit; i++) {
          if (strncmp(percpu_arena_mode_names[i], v, vlen) == 0) {
            if (!have_percpu_arena) {
              malloc_conf_error("No getcpu support", k, klen, v, vlen);
            }
            opt_percpu_arena = i;
            match = true;
            break;
      ...
      CONF_HANDLE_SIZE_T(opt_max_background_threads, "max_background_threads", 1,
                         opt_max_background_threads, yes, yes, true);
      if (config_prof) {
        ...
        CONF_HANDLE_SIZE_T(opt_lg_prof_sample, "lg_prof_sample", 0, (sizeof(uint64_t) << 3) - 1, no,
                           yes, true)
        ...
        for (int i = 0; i < thp_mode_names_limit; i++) {
          if (strncmp(thp_mode_names[i], v, vlen) == 0) {
            if (!have_madvise_huge) {
              malloc_conf_error("No THP support", k, klen, v, vlen);
            }
            opt_thp = i;
            match = true;
            break;
      ...
      malloc_conf_error("Invalid conf pair", k, klen, v, vlen);
#undef CONF_MATCH
...
static bool malloc_init_hard_recursible(void) {
  ...
  ncpus = malloc_ncpus();
     ...
     !defined(JEMALLOC_ZONE) && !defined(_WIN32) && !defined(__native_client__))
...
JEMALLOC_ALWAYS_INLINE void *imalloc_no_sample(static_opts_t *sopts, dynamic_opts_t *dopts,
                                               tsd_t *tsd, size_t size, size_t usize, szind_t ind) {
  tcache_t *tcache;
  arena_t *arena;
...
JEMALLOC_ALWAYS_INLINE void *imalloc_sample(static_opts_t *sopts, dynamic_opts_t *dopts, tsd_t *tsd,
  ...
  if (usize <= SMALL_MAXCLASS) {
    assert(((dopts->alignment == 0) ? sz_s2u(LARGE_MINCLASS)
                                    : sz_sa2u(LARGE_MINCLASS, dopts->alignment)) == LARGE_MINCLASS);
 ...
 */
JEMALLOC_ALWAYS_INLINE bool compute_size_with_overflow(bool may_overflow, dynamic_opts_t *dopts,
                                                       size_t *size) {
  /*
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *JEMALLOC_ATTR(
    malloc) JEMALLOC_ALLOC_SIZE(1) je_malloc(size_t size) {
  void *ret;
  static_opts_t sopts;
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *JEMALLOC_ATTR(
    malloc) JEMALLOC_ALLOC_SIZE(2) je_aligned_alloc(size_t alignment, size_t size) {
  void *ret;
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *JEMALLOC_ATTR(
    malloc) JEMALLOC_ALLOC_SIZE2(1, 2) je_calloc(size_t num, size_t size) {
  void *ret;
  static_opts_t sopts;
  dynamic_opts_t dopts;
...
static void *irealloc_prof_sample(tsd_t *tsd, void *old_ptr, size_t old_usize, size_t usize,
                                  prof_tctx_t *tctx) {
  void *p;
...
JEMALLOC_ALWAYS_INLINE void *irealloc_prof(tsd_t *tsd, void *old_ptr, size_t old_usize,
                                           size_t usize, alloc_ctx_t *alloc_ctx) {
  void *p;
  bool prof_active;
...
JEMALLOC_ALWAYS_INLINE void isfree(tsd_t *tsd, void *ptr, size_t usize, tcache_t *tcache,
                                   bool slow_path) {
  if (!slow_path) {
    tsd_assert_fast(tsd);
  }
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *
  ...
  if (likely(ptr != NULL)) {
    ...
    if (config_prof && opt_prof) {
      ...
      ret = unlikely(usize == 0 || usize > LARGE_MAXCLASS)
                ? NULL
                : irealloc_prof(tsd, ptr, old_usize, usize, &alloc_ctx);
    } else {
      if (config_stats) {
        usize = sz_s2u(size);
      }
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *JEMALLOC_ATTR(
    malloc) je_memalign(size_t alignment, size_t size) {
  void *ret;
  static_opts_t sopts;
  dynamic_opts_t dopts;
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *JEMALLOC_ATTR(
    malloc) JEMALLOC_ALLOC_SIZE(1) je_mallocx(size_t size, int flags) {
  void *ret;
  static_opts_t sopts;
  dynamic_opts_t dopts;
  ...
  if (unlikely(flags != 0)) {
    ...
    if ((flags & MALLOCX_ARENA_MASK) != 0) dopts.arena_ind = MALLOCX_ARENA_GET(flags);
  }
...
static void *irallocx_prof_sample(tsdn_t *tsdn, void *old_ptr, size_t old_usize, size_t usize,
  ...
  if (usize <= SMALL_MAXCLASS) {
    p = iralloct(tsdn, old_ptr, old_usize, LARGE_MINCLASS, alignment, zero, tcache, arena);
    if (p == NULL) {
      return NULL;
    }
    arena_prof_promote(tsdn, p, usize);
  } else {
    p = iralloct(tsdn, old_ptr, old_usize, usize, alignment, zero, tcache, arena);
  }

  return p;
}

JEMALLOC_ALWAYS_INLINE void *irallocx_prof(tsd_t *tsd, void *old_ptr, size_t old_usize, size_t size,
                                           size_t alignment, size_t *usize, bool zero,
  ...
  if (unlikely((uintptr_t)tctx != (uintptr_t)1U)) {
    p = irallocx_prof_sample(tsd_tsdn(tsd), old_ptr, old_usize, *usize, alignment, zero, tcache,
                             arena, tctx);
  } else {
    p = iralloct(tsd_tsdn(tsd), old_ptr, old_usize, size, alignment, zero, tcache, arena);
  }
  if (unlikely(p == NULL)) {
...
JEMALLOC_ALLOC_SIZE(2) je_rallocx(void *ptr, size_t size, int flags) {
  ...
  if (config_prof && opt_prof) {
    ...
    p = irallocx_prof(tsd, ptr, old_usize, size, alignment, &usize, zero, tcache, arena,
                      &alloc_ctx);
    if (unlikely(p == NULL)) {
      goto label_oom;
    }
  } else {
    p = iralloct(tsd_tsdn(tsd), ptr, old_usize, size, alignment, zero, tcache, arena);
    if (unlikely(p == NULL)) {
      goto label_oom;
    }
    if (config_stats) {
...
JEMALLOC_ALWAYS_INLINE size_t ixallocx_helper(tsdn_t *tsdn, void *ptr, size_t old_usize,
                                              size_t size, size_t extra, size_t alignment,
                                              bool zero) {
  size_t usize;
...
static size_t ixallocx_prof_sample(tsdn_t *tsdn, void *ptr, size_t old_usize, size_t size,
...
JEMALLOC_ALWAYS_INLINE size_t ixallocx_prof(tsd_t *tsd, void *ptr, size_t old_usize, size_t size,
...
JEMALLOC_EXPORT size_t JEMALLOC_NOTHROW je_xallocx(void *ptr, size_t size, size_t extra,
                                                   int flags) {
  tsd_t *tsd;
...
JEMALLOC_EXPORT size_t JEMALLOC_NOTHROW JEMALLOC_ATTR(pure) je_nallocx(size_t size, int flags) {
  size_t usize;
  tsdn_t *tsdn;
...
JEMALLOC_EXPORT int JEMALLOC_NOTHROW je_mallctl(const char *name, void *oldp, size_t *oldlenp,
...
JEMALLOC_EXPORT int JEMALLOC_NOTHROW je_mallctlnametomib(const char *name, size_t *mibp,
                                                         size_t *miblenp) {
  int ret;
...
/* Helps the application decide if a pointer is worth re-allocating in order to reduce
 * fragmentation. returns 1 if the allocation should be moved, and 0 if the allocation be kept. If
 * the application decides to re-allocate it should use MALLOCX_TCACHE_NONE when doing so. */
JEMALLOC_EXPORT int JEMALLOC_NOTHROW get_defrag_hint(void *ptr) {
  assert(ptr != NULL);
