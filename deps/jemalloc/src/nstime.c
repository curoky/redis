...
void nstime_init2(nstime_t *time, uint64_t sec, uint64_t nsec) { time->ns = sec * BILLION + nsec; }
...
void nstime_copy(nstime_t *time, const nstime_t *source) { *time = *source; }
...
void nstime_imultiply(nstime_t *time, uint64_t multiplier) {
  assert((((time->ns | multiplier) & (UINT64_MAX << (sizeof(uint64_t) << 2))) == 0) ||
...
#define NSTIME_MONOTONIC true
static void nstime_get(nstime_t *time) { nstime_init(time, mach_absolute_time()); }
#else
