...
static void gen_mutex_ctl_str(char *str, size_t buf_len, const char *prefix, const char *mutex,
                              const char *counter) {
  malloc_snprintf(str, buf_len, "stats.%s.%s.%s", prefix, mutex, counter);
}

static void mutex_stats_init_cols(emitter_row_t *row, const char *table_name, emitter_col_t *name,
...
static void mutex_stats_read_global(const char *name, emitter_col_t *col_name,
                                    emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
                                    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters]) {
...
static void mutex_stats_read_arena(unsigned arena_ind, mutex_prof_arena_ind_t mutex_ind,
...
static void mutex_stats_read_arena_bin(
    unsigned arena_ind, unsigned bin_ind,
    emitter_col_t col_uint64_t[mutex_prof_num_uint64_t_counters],
    emitter_col_t col_uint32_t[mutex_prof_num_uint32_t_counters]) {
  char cmd[MUTEX_CTL_STR_MAX_LENGTH];
  emitter_col_t *dst;
...
static void stats_arena_bins_print(emitter_t *emitter, bool mutex, unsigned i) {
  size_t page;
...
#undef COL
  ...
  for (j = 0, in_gap = false; j < nbins; j++) {
    ...
    if (mutex) {
      mutex_stats_read_arena_bin(i, j, col_mutex64, col_mutex32);
    }
...
static void stats_arena_lextents_print(emitter_t *emitter, unsigned i) {
...
static void stats_arena_mutexes_print(emitter_t *emitter, unsigned arena_ind) {
  emitter_row_t row;
  ...
  for (mutex_prof_arena_ind_t i = 0; i < mutex_prof_num_arena_mutexes; i++) {
    ...
    mutex_stats_read_arena(arena_ind, i, name, &col_name, col64, col32);
...
static void stats_arena_print(emitter_t *emitter, unsigned i, bool bins, bool large, bool mutex) {
  unsigned nthreads;
  const char *dss;
...
#define GET_AND_EMIT_ALLOC_STAT(small_or_large, name, valtype)                         \
  CTL_M2_GET("stats.arenas.0." #small_or_large "." #name, i, &small_or_large##_##name, \
             valtype##_t);                                                             \
...
#define OPT_WRITE_MUTABLE(name, var1, var2, size, emitter_type, altname)                    \
  if (je_mallctl("opt." name, (void *)&var1, &size, NULL, 0) == 0 &&                        \
    ...
    emitter_kv_note(emitter, name, "opt." name, emitter_type, &var1, altname, emitter_type, \
                    &var2);                                                                 \
  }
...
#undef OPT_WRITE_CHAR_P
  ...
  unsigned nlextents;
  CTL_GET("arenas.nlextents", &nlextents, unsigned);
  emitter_kv(emitter, "nlextents", "Number of large size classes", emitter_type_unsigned,
             &nlextents);
...
static void stats_print_helper(emitter_t *emitter, bool merged, bool destroyed, bool unmerged,
                               bool bins, bool large, bool mutex) {
  /*
  ...
  if (mutex) {
    ...
    for (int i = 0; i < mutex_prof_num_global_mutexes; i++) {
      mutex_stats_read_global(global_mutex_names[i], &name, col64, col32);
  ...
  if (merged || destroyed || unmerged) {
    ...
    if (merged && (ninitialized > 1 || !unmerged)) {
      ...
      stats_arena_print(emitter, MALLCTL_ARENAS_ALL, bins, large, mutex);
    ...
    if (destroyed_initialized && destroyed) {
      ...
      stats_arena_print(emitter, MALLCTL_ARENAS_DESTROYED, bins, large, mutex);
    ...
    if (unmerged) {
      for (i = j = 0; i < narenas; i++) {
        if (initialized[i]) {
          ...
          emitter_json_dict_begin(emitter, arena_ind_str);
          emitter_table_printf(emitter, "arenas[%s]:\n", arena_ind_str);
          stats_arena_print(emitter, i, bins, large, mutex);
          /* Close "<arena-ind>". */
          emitter_json_dict_end(emitter);
        }
      }
...
#undef OPTION
  ...
  if (config_stats) {
    stats_print_helper(&emitter, merged, destroyed, unmerged, bins, large, mutex);
  }
