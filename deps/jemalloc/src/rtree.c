...
UNUSED rtree_node_dalloc_t *JET_MUTABLE rtree_node_dalloc = rtree_node_dalloc_impl;
...
UNUSED rtree_leaf_dalloc_t *JET_MUTABLE rtree_leaf_dalloc = rtree_leaf_dalloc_impl;

#ifdef JEMALLOC_JET
#if RTREE_HEIGHT > 1
static void rtree_delete_subtree(tsdn_t *tsdn, rtree_t *rtree, rtree_node_elm_t *subtree,
                                 unsigned level) {
...
static rtree_node_elm_t *rtree_node_init(tsdn_t *tsdn, rtree_t *rtree, unsigned level,
                                         atomic_p_t *elmp) {
...
static bool rtree_node_valid(rtree_node_elm_t *node) { return ((uintptr_t)node != (uintptr_t)0); }

static bool rtree_leaf_valid(rtree_leaf_elm_t *leaf) { return ((uintptr_t)leaf != (uintptr_t)0); }
...
static rtree_node_elm_t *rtree_child_node_read(tsdn_t *tsdn, rtree_t *rtree, rtree_node_elm_t *elm,
...
static rtree_leaf_elm_t *rtree_child_leaf_read(tsdn_t *tsdn, rtree_t *rtree, rtree_node_elm_t *elm,
                                               unsigned level, bool dependent) {
  rtree_leaf_elm_t *leaf;
...
#define RTREE_GET_CHILD(level)                                                                  \
  {                                                                                             \
    ...
    if (level + 2 < RTREE_HEIGHT) {                                                             \
      node = init_missing ? rtree_child_node_read(tsdn, rtree, &node[subkey], level, dependent) \
                          : rtree_child_node_tryread(&node[subkey], dependent);                 \
    } else {                                                                                    \
      leaf = init_missing ? rtree_child_leaf_read(tsdn, rtree, &node[subkey], level, dependent) \
