 ...
 */
static bool ckh_evict_reloc_insert(ckh_t *ckh, size_t argbucket, void const **argkey,
                                   void const **argdata) {
...
static bool ckh_grow(tsd_t *tsd, ckh_t *ckh) {
  ...
  while (true) {
    ...
    tab = (ckhc_t *)ipallocztm(tsd_tsdn(tsd), usize, CACHELINE, true, NULL, true,
...
bool ckh_new(tsd_t *tsd, ckh_t *ckh, size_t minitems, ckh_hash_t *hash, ckh_keycomp_t *keycomp) {
  ...
  for (lg_mincells = LG_CKH_BUCKET_CELLS; (ZU(1) << lg_mincells) < mincells; lg_mincells++) {
    /* Do nothing. */
  }
  ...
  ckh->tab = (ckhc_t *)ipallocztm(tsd_tsdn(tsd), usize, CACHELINE, true, NULL, true,
                                  arena_ichoose(tsd, NULL));
  if (ckh->tab == NULL) {
    ret = true;
    goto label_return;
...
void ckh_delete(tsd_t *tsd, ckh_t *ckh) {
  ...
  malloc_printf("%s(%p): ngrows: %" FMTu64 ", nshrinks: %" FMTu64
                ...
                ","
                " nrelocs: %" FMTu64 "\n",
                __func__, ckh, (unsigned long long)ckh->ngrows, (unsigned long long)ckh->nshrinks,
                (unsigned long long)ckh->nshrinkfails, (unsigned long long)ckh->ninserts,
...
bool ckh_iter(ckh_t *ckh, size_t *tabind, void **key, void **data) {
  ...
  for (i = *tabind, ncells = (ZU(1) << (ckh->lg_curbuckets + LG_CKH_BUCKET_CELLS)); i < ncells;
       i++) {
...
bool ckh_remove(tsd_t *tsd, ckh_t *ckh, const void *searchkey, void **key, void **data) {
  ...
  if (cell != SIZE_T_MAX) {
    ...
    if (ckh->count < (ZU(1) << (ckh->lg_curbuckets + LG_CKH_BUCKET_CELLS - 2)) &&
...
bool ckh_pointer_keycomp(const void *k1, const void *k2) { return (k1 == k2); }
