...
#define SC(index, lg_grp, lg_delta, ndelta, psz, bin, pgs, lg_delta_lookup)   \
  BIN_INFO_bin_##bin((1U << lg_grp) + (ndelta << lg_delta), (pgs << LG_PAGE), \
                     (pgs << LG_PAGE) / ((1U << lg_grp) + (ndelta << lg_delta)))
    SIZE_CLASSES
...
void bin_prefork(tsdn_t *tsdn, bin_t *bin) { malloc_mutex_prefork(tsdn, &bin->lock); }

void bin_postfork_parent(tsdn_t *tsdn, bin_t *bin) {
  malloc_mutex_postfork_parent(tsdn, &bin->lock);
}

void bin_postfork_child(tsdn_t *tsdn, bin_t *bin) { malloc_mutex_postfork_child(tsdn, &bin->lock); }
