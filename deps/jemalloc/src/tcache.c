...
size_t tcache_salloc(tsdn_t *tsdn, const void *ptr) { return arena_salloc(tsdn, ptr); }
...
void *tcache_alloc_small_hard(tsdn_t *tsdn, arena_t *arena, tcache_t *tcache, cache_bin_t *tbin,
                              szind_t binind, bool *tcache_success) {
  void *ret;
...
void tcache_bin_flush_small(tsd_t *tsd, tcache_t *tcache, cache_bin_t *tbin, szind_t binind,
                            unsigned rem) {
  ...
  while (nflush > 0) {
    ...
    for (unsigned i = 0; i < nflush; i++) {
      ...
      if (extent_arena_get(extent) == bin_arena) {
        arena_dalloc_bin_junked_locked(tsd_tsdn(tsd), bin_arena, extent, ptr);
      } else {
        /*
...
void tcache_bin_flush_large(tsd_t *tsd, cache_bin_t *tbin, szind_t binind, unsigned rem,
                            tcache_t *tcache) {
  bool merged_stats = false;
  ...
  while (nflush > 0) {
    ...
    if ((config_prof || config_stats) && locked_arena == arena) {
      if (config_prof) {
        idump = arena_prof_accum(tsd_tsdn(tsd), arena, tcache->prof_accumbytes);
        tcache->prof_accumbytes = 0;
      }
      if (config_stats) {
        merged_stats = true;
        arena_stats_large_nrequests_add(tsd_tsdn(tsd), &arena->stats, binind,
  ...
  if (config_stats && !merged_stats) {
     ...
     * arena, so the stats didn't get merged.  Manually do so now.
     */
    arena_stats_large_nrequests_add(tsd_tsdn(tsd), &arena->stats, binind, tbin->tstats.nrequests);
    tbin->tstats.nrequests = 0;
  }
...
bool tsd_tcache_data_init(tsd_t *tsd) {
  ...
  void *avail_array =
      ipallocztm(tsd_tsdn(tsd), size, CACHELINE, true, NULL, true, arena_get(TSDN_NULL, 0, true));
  if (avail_array == NULL) {
    return true;
  }
...
static void tcache_destroy(tsd_t *tsd, tcache_t *tcache, bool tsd_tcache) {
  ...
  if (tsd_tcache) {
    ...
    void *avail_array = (void *)((uintptr_t)tcache_small_bin_get(tcache, 0)->avail -
...
void tcache_stats_merge(tsdn_t *tsdn, tcache_t *tcache, arena_t *arena) {
  ...
  for (; i < nhbins; i++) {
    cache_bin_t *tbin = tcache_large_bin_get(tcache, i);
    arena_stats_large_nrequests_add(tsdn, &arena->stats, i, tbin->tstats.nrequests);
    tbin->tstats.nrequests = 0;
  }
}
...
static bool tcaches_create_prep(tsd_t *tsd) {
  ...
  if (tcaches == NULL) {
    tcaches = base_alloc(tsd_tsdn(tsd), b0get(), sizeof(tcache_t *) * (MALLOCX_TCACHE_MAX + 1),
                         CACHELINE);
    if (tcaches == NULL) {
      err = true;
      goto label_return;
...
static tcache_t *tcaches_elm_remove(tsd_t *tsd, tcaches_t *elm) {
...
bool tcache_boot(tsdn_t *tsdn) {
  ...
  tcache_bin_info =
      (cache_bin_info_t *)base_alloc(tsdn, b0get(), nhbins * sizeof(cache_bin_info_t), CACHELINE);
