...
void bitmap_init(bitmap_t *bitmap, const bitmap_info_t *binfo, bool fill) {
  ...
  for (i = 1; i < binfo->nlevels; i++) {
    ...
    extra =
        (BITMAP_GROUP_NBITS - (group_count & BITMAP_GROUP_NBITS_MASK)) & BITMAP_GROUP_NBITS_MASK;
    if (extra != 0) {
...
static size_t bitmap_info_ngroups(const bitmap_info_t *binfo) { return binfo->ngroups; }
