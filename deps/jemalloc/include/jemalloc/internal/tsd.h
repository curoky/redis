...
#define TSD_INITIALIZER                                                                   \
  {                                                                                       \
    tsd_state_uninitialized, TCACHE_ENABLED_ZERO_INITIALIZER, false, 0, 0, 0, 0, 0, NULL, \
        RTREE_CTX_ZERO_INITIALIZER, NULL, NULL, NULL, TCACHE_ZERO_INITIALIZER,            \
...
struct tsd_s {
   ...
   */
  tsd_state_t state;
#define O(n, t, nt) t use_a_getter_or_setter_instead_##n;
  MALLOC_TSD
#undef O
};
...
JEMALLOC_ALWAYS_INLINE bool tsdn_null(const tsdn_t *tsdn) { return tsdn == NULL; }
...
#define O(n, t, nt)                                                                              \
  JEMALLOC_ALWAYS_INLINE t *tsd_##n##p_get(tsd_t *tsd) {                                         \
    assert(tsd->state == tsd_state_nominal || tsd->state == tsd_state_nominal_slow ||            \
           tsd->state == tsd_state_reincarnated || tsd->state == tsd_state_minimal_initialized); \
...
JEMALLOC_ALWAYS_INLINE tsd_t *tsd_fetch_min(void) { return tsd_fetch_impl(true, true); }
...
JEMALLOC_ALWAYS_INLINE tsd_t *tsd_fetch(void) { return tsd_fetch_impl(true, false); }

static inline bool tsd_nominal(tsd_t *tsd) { return (tsd->state <= tsd_state_nominal_max); }
...
JEMALLOC_ALWAYS_INLINE rtree_ctx_t *tsd_rtree_ctx(tsd_t *tsd) { return tsd_rtree_ctxp_get(tsd); }
