...
int test_threads() {
  ...
  static const int sizes[] = {7,    16,   32,    60,    91,    100,    120,    144,
                              169,  199,  255,   400,   670,   900,    917,    1025,
                              3333, 5190, 13131, 49192, 99999, 123123, 255265, 2333111};
        ...
        thread t([&]() {
          for (int i = 0; i < numIter2; ++i) {
            ...
            for (int j = 0; j < numAllocs; j += 64) {
              ...
              for (int k = 0; k < sz; k++) ptrs[j][k] = tid + k;
            }
            for (int j = 0; j < numAllocs; j += 64) {
  ...
  bool failed =
      leaked > 65536;  // in case C++ runtime allocated something (e.g. iostream locale or facet)
