...
void get_redis_version(redisContext *c, int *majorptr, int *minorptr) {
  ...
  if (reply == NULL || c->err || reply->type != REDIS_REPLY_STRING) goto abort;
  if ((s = strstr(reply->str, REDIS_VERSION_FIELD)) == NULL) goto abort;

  s += strlen(REDIS_VERSION_FIELD);

  /* We need a field terminator and at least 'x.y.z' (5) bytes of data */
  if ((e = strstr(s, "\r\n")) == NULL || (e - s) < 5) goto abort;
...
static int disconnect(redisContext *c, int keep_fd) {
  ...
  if (keep_fd) return redisFreeKeepFd(c);
  redisFree(c);
  return -1;
}
...
static void test_blocking_connection(struct config config) {
  ...
  test_cond(reply->type == REDIS_REPLY_STATUS && strcasecmp(reply->str, "pong") == 0)
      freeReplyObject(reply);
  ...
  test_cond(reply->type == REDIS_REPLY_ARRAY && reply->elements == 2 &&
            !memcmp(reply->element[0]->str, "bar", 3) && !memcmp(reply->element[1]->str, "foo", 3))
      freeReplyObject(reply);
