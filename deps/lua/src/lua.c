...
static void print_version(void) { l_message(NULL, LUA_RELEASE "  " LUA_COPYRIGHT); }
  ...
  while ((status = loadline(L)) != -1) {
    ...
    if (status == 0 && lua_gettop(L) > 0) { /* any result to print? */
      ...
      if (lua_pcall(L, lua_gettop(L) - 1, 0, 0) != 0)
        l_message(progname, lua_pushfstring(L, "error calling " LUA_QL("print") " (%s)",
                                            lua_tostring(L, -1)));
    }
  }
...
static int handle_script(lua_State *L, char **argv, int n) {
  ...
  lua_setglobal(L, "arg");
  fname = argv[n];
  if (strcmp(fname, "-") == 0 && strcmp(argv[n - 1], "--") != 0) fname = NULL; /* stdin */
