...
void replBackupCallback(RedisModuleCtx *ctx, RedisModuleEvent e, uint64_t sub, void *data) {
  ...
  switch (sub) {
    ...
    case REDISMODULE_SUBEVENT_REPL_BACKUP_RESTORE:
      if (before_str) RedisModule_FreeString(ctx, before_str);
      if (after_str) RedisModule_FreeString(ctx, after_str);
    ...
    case REDISMODULE_SUBEVENT_REPL_BACKUP_DISCARD:
      if (before_str_backup) RedisModule_FreeString(ctx, before_str_backup);
      if (after_str_backup) RedisModule_FreeString(ctx, after_str_backup);
      before_str_backup = NULL;
      after_str_backup = NULL;
      break;
    default:
...
void *testrdb_type_load(RedisModuleIO *rdb, int encver) {
  ...
  if (RedisModule_IsIOError(rdb)) {
    ...
    if (str) RedisModule_FreeString(ctx, str);
    return NULL;
  }
...
int testrdb_aux_load(RedisModuleIO *rdb, int encver, int when) {
  ...
  if (when == REDISMODULE_AUX_BEFORE_RDB) {
    ...
    if (count) before_str = RedisModule_LoadString(rdb);
  } else {
    if (after_str) RedisModule_FreeString(ctx, after_str);
    after_str = NULL;
    int count = RedisModule_LoadSigned(rdb);
    if (RedisModule_IsIOError(rdb)) return REDISMODULE_ERR;
    if (count) after_str = RedisModule_LoadString(rdb);
  }
  if (RedisModule_IsIOError(rdb)) return REDISMODULE_ERR;
...
void testrdb_type_free(void *value) {
  if (value) RedisModule_FreeString(NULL, (RedisModuleString *)value);
}
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (argc > 0) RedisModule_StringToLongLong(argv[0], &conf_aux_count);
  ...
  if (conf_aux_count == 0) {
    ...
    if (testrdb_type == NULL) return REDISMODULE_ERR;
  } else {
    RedisModuleTypeMethods datatype_methods = {
        ...
        .aux_save_triggers =
            (conf_aux_count == 1 ? REDISMODULE_AUX_AFTER_RDB
  ...
  if (RedisModule_CreateCommand(ctx, "testrdb.set.before", testrdb_set_before, "deny-oom", 0, 0,
                                0) == REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "testrdb.get.before", testrdb_get_before, "", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "testrdb.set.after", testrdb_set_after, "deny-oom", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "testrdb.get.after", testrdb_get_after, "", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "testrdb.set.key", testrdb_set_key, "deny-oom", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "testrdb.get.key", testrdb_get_key, "", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  RedisModule_SubscribeToServerEvent(ctx, RedisModuleEvent_ReplBackup, replBackupCallback);
