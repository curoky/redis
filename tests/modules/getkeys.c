...
int getkeys_command(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (RedisModule_IsKeysPositionRequest(ctx)) {
    for (i = 0; i < argc; i++) {
      ...
      if (len == 3 && !strncasecmp(str, "key", 3) && i + 1 < argc) RedisModule_KeyAtPos(ctx, i + 1);
    }
...
int getkeys_introspect(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!keyidx) {
    ...
    for (i = 0; i < num_keys; i++) RedisModule_ReplyWithString(ctx, argv[1 + keyidx[i]]);
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (RedisModule_CreateCommand(ctx, "getkeys.command", getkeys_command, "getkeys-api", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "getkeys.fixed", getkeys_fixed, "", 2, 4, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "getkeys.introspect", getkeys_introspect, "", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;
