...
void LazyFreeLinkFree(void *value) { LazyFreeLinkReleaseObject(value); }
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  /* We only allow our module to be loaded when the redis core version is greater than the version
   * of my module */
  ...
  if (RedisModule_CreateCommand(ctx, "lazyfreelink.insert", LazyFreeLinkInsert_RedisCommand,
                                "write deny-oom", 1, 1, 1) == REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "lazyfreelink.len", LazyFreeLinkLen_RedisCommand, "readonly",
