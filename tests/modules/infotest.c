...
int info_get(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, char field_type) {
  ...
  if (field_type == 'i') {
    ...
    if (err == REDISMODULE_OK) RedisModule_ReplyWithLongLong(ctx, ll);
  } else if (field_type == 'u') {
    unsigned long long ll =
        (unsigned long long)RedisModule_ServerInfoGetFieldUnsigned(info, field, &err);
    if (err == REDISMODULE_OK) RedisModule_ReplyWithLongLong(ctx, ll);
  } else if (field_type == 'd') {
    double d = RedisModule_ServerInfoGetFieldDouble(info, field, &err);
    if (err == REDISMODULE_OK) RedisModule_ReplyWithDouble(ctx, d);
  } else if (field_type == 'c') {
    const char *str = RedisModule_ServerInfoGetFieldC(info, field);
    if (str) RedisModule_ReplyWithCString(ctx, str);
  } else {
  ...
  if (err != REDISMODULE_OK) RedisModule_ReplyWithError(ctx, "not found");
