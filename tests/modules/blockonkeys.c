...
void fsl_type_free(fsl_t *o) { RedisModule_Free(o); }
...
void fsl_free(void *value) { fsl_type_free(value); }

/* ========================== helper methods ======================= */

int get_fsl(RedisModuleCtx *ctx, RedisModuleString *keyname, int mode, int create, fsl_t **fsl,
            int reply_on_failure) {
  RedisModuleKey *key = RedisModule_OpenKey(ctx, keyname, mode);

  int type = RedisModule_KeyType(key);
  if (type != REDISMODULE_KEYTYPE_EMPTY && RedisModule_ModuleTypeGetType(key) != fsltype) {
    RedisModule_CloseKey(key);
    if (reply_on_failure) RedisModule_ReplyWithError(ctx, REDISMODULE_ERRORMSG_WRONGTYPE);
...
int fsl_push(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  fsl_t *fsl;
  if (!get_fsl(ctx, argv[1], REDISMODULE_WRITE, 1, &fsl, 1)) return REDISMODULE_OK;

  if (fsl->length == LIST_SIZE) return RedisModule_ReplyWithError(ctx, "ERR list is full");
...
int bpop_reply_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  fsl_t *fsl;
  if (!get_fsl(ctx, keyname, REDISMODULE_READ, 0, &fsl, 0) || !fsl) return REDISMODULE_ERR;
...
int fsl_bpop(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  fsl_t *fsl;
  if (!get_fsl(ctx, argv[1], REDISMODULE_READ, 0, &fsl, 1)) return REDISMODULE_OK;

  if (!fsl) {
    RedisModule_BlockClientOnKeys(ctx, bpop_reply_callback, bpop_timeout_callback, NULL, timeout,
                                  &argv[1], 1, NULL);
  } else {
    RedisModule_ReplyWithLongLong(ctx, fsl->list[--fsl->length]);
  }
...
int bpopgt_reply_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (fsl->list[fsl->length - 1] <= *pgt) return REDISMODULE_ERR;
...
int bpoppush_reply_callback(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  fsl_t *src;
  if (!get_fsl(ctx, src_keyname, REDISMODULE_READ, 0, &src, 0) || !src) return REDISMODULE_ERR;

  fsl_t *dst;
  if (!get_fsl(ctx, dst_keyname, REDISMODULE_WRITE, 1, &dst, 0) || !dst) return REDISMODULE_ERR;
...
int fsl_bpoppush(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  fsl_t *src;
  if (!get_fsl(ctx, argv[1], REDISMODULE_READ, 0, &src, 1)) return REDISMODULE_OK;
  ...
  if (!src) {
    ...
    fsl_t *dst;
    if (!get_fsl(ctx, argv[2], REDISMODULE_WRITE, 1, &dst, 1)) return REDISMODULE_OK;
    long long ele = src->list[--src->length];
    dst->list[dst->length++] = ele;
...
int fsl_getall(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (!fsl) return RedisModule_ReplyWithArray(ctx, 0);
...
int blockonkeys_popall(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (RedisModule_KeyType(key) == REDISMODULE_KEYTYPE_EMPTY) {
    RedisModule_BlockClientOnKeys(ctx, blockonkeys_popall_reply_callback,
                                  blockonkeys_popall_timeout_callback, NULL, 3000, &argv[1], 1,
                                  NULL);
  } else {
...
int blockonkeys_blpopn(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (keytype != REDISMODULE_KEYTYPE_EMPTY && keytype != REDISMODULE_KEYTYPE_LIST) {
    ...
    RedisModule_BlockClientOnKeys(ctx, blockonkeys_blpopn_reply_callback,
                                  blockonkeys_blpopn_timeout_callback, NULL, 3000, &argv[1], 1,
                                  NULL);
  }
  RedisModule_CloseKey(key);
  return REDISMODULE_OK;
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (fsltype == NULL) return REDISMODULE_ERR;
  ...
  if (RedisModule_CreateCommand(ctx, "blockonkeys.popall", blockonkeys_popall, "", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "blockonkeys.lpush", blockonkeys_lpush, "", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "blockonkeys.lpush_unblock", blockonkeys_lpush, "", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;

  if (RedisModule_CreateCommand(ctx, "blockonkeys.blpopn", blockonkeys_blpopn, "", 1, 1, 1) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;
