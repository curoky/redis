...
void scan_strings_callback(RedisModuleCtx *ctx, RedisModuleString *keyname, RedisModuleKey *key,
                           void *privdata) {
  ...
  if (was_opened) RedisModule_CloseKey(key);
}
...
void scan_key_callback(RedisModuleKey *key, RedisModuleString *field, RedisModuleString *value,
                       void *privdata) {
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
  ...
  if (RedisModule_CreateCommand(ctx, "scan.scan_strings", scan_strings, "", 0, 0, 0) ==
      REDISMODULE_ERR)
    return REDISMODULE_ERR;
